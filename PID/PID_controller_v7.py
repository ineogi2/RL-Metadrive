...
def return_pos(current_pos, x_dir, y_dir, dx, dy):
    dx_wp = dx*x_dir
    dy_wp = dy*y_dir
    # print(current_pos, x_dir, y_dir, dx, dy, current_pos+dx_wp+dy_wp)
    return current_pos+dx_wp+dy_wp
...
class Controller():
    ...
    def _update_values(self, state):
        ...
        self._aim_speed = 7*state._max_dist
...
class State():
    ...
    def _update_waypoints(self, rel_waypoints):
        ...
        for i in range(len(rel_waypoints)//2):
            waypoint = return_pos(now, np.array(self._direction), np.array(self._y_direction), rel_waypoints[2*i], rel_waypoints[2*i+1])
            waypoints.append(waypoint)

        # print(waypoints)
        self._waypoints = waypoints[1:]
        self._min_dist = norm_sq(now, self._waypoints[0])
        self._max_dist = norm_sq(now, self._waypoints[-1])
    ...
    def __str__(self) -> str:
        return f"xy : {(self._current_x, self._current_y)} / cur_speed : {self._current_speed} / " \
            + f"aim_speed : {(self._max_dist**0.5)*7}"
