...
class PID_controller:
    def __init__(self, speed_gain=[1,0,0.01], steering_gain=[1,0,0.1]):
    ...
    def lane_keeping(self):
        steering_angle = self._pid_result(self.steering_gain, self.dir_err) if abs(self.dir_err[0])>0.9 else self.heading*0.1
        acc = self._pid_result(self.speed_gain, self.speed_err)
  
        return [steering_angle, acc]

    def lane_change(self, direction, cur_speed):
        steering_angle = direction*0.1
        acc = self._pid_result(self.speed_gain, self.speed_err)
        steps = 0
